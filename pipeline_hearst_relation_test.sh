...
# Arguments 
######################################################
ltf_source=hearst_data/hearst_files/ # ltf source file path
file_list=hearst_data/hearst_files_lst # file list
edl_output_dir=hearst_data/edl/ # edl output
edl_tab=${edl_output_dir}merged.tab   # linking tab file path
edl_cs=${edl_output_dir}merged.cs # linking cs filt path
relation_result_dir=hearst_data/relation/   # final cs output file path
relation_cs_name=en.rel.cs # final cs output for relation
event_dep_dir=hearst_data/dep # dependency parser outout
event_tmp_tdf_dir=hearst_data/tdf # text data feeder folder for events
event_no_format_dir=hearst_data/event_no_format #unformatted event output
event_result_dir=hearst_data/event/ #event output folder
final_output_file=hearst_data/en_full.cs
...
# Relation
# docker run --runtime=nvidia -it --rm -v $PWD:/tmp -w /tmp rpi/relation \
