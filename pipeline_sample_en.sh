...
link_dir=linking/edl_data/test
mkdir -p ${PWD}/${link_dir}
cp -r ${edl_output_dir} ${PWD}/${link_dir}/input
docker run --user "$(id -u):$(id -g)" --rm -v ${PWD}/linking/edl_data:/data  --link db:mongo panx27/edl \
...
docker run --user "$(id -u):$(id -g)" --rm -v `pwd`:`pwd` -w `pwd` -i -t limanling/uiuc_ie_m18 \
    /opt/conda/envs/aida_entity/bin/python \
    ./aida_pipeline_m18/aida_event/fine_grained_events.py \
