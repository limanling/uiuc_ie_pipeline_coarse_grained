...
# tab_file = "/nas/data/m1/panx2/tmp/aida/eval/2018/en/0918/en.linking.corf.tab"
# result_dir = "/data/m1/shig4/AIDA/results/fix/ru/"
cs_file = output_params.edl_cs
plain_text = output_params.aida_plain_text
results_file = output_params.aida_results
output_cs = output_params.rel_cs
id_relation = {}
...
results = []
score = []
with open(results_file) as fmodel:
    for line in fmodel:
...
mention1 = []
mention2 = []
output = open(output_cs, "w")
i = 0

###########################
# map doc:offset to string
###########################
# offset2str = {}
# with open(tab_file) as fmodel:
#     for line in fmodel:
#         whole_sentence = line.strip().split("\t")
#         doc_id = whole_sentence[3]
#         string = whole_sentence[2]
#         if doc_id not in offset2str:
#             offset2str[doc_id] = string
score_idx = 0
query_id = 0
###########################
# mention ID to KB ID
...
with open(plain_text) as fmodel:
    for line in fmodel:
        ...
        if ERE_to_AIDA_final_ouput[results[i]] != 'NO-RELATION(Arg-1,Arg-1)' and mid2kbid[mention1] != mid2kbid[mention2]:
            ...
            # output.write("EN_RELATION_%04d"%query_id + "\t" +results[i] + "\t" + "dryrun0" + "\t" + seg_offset + "\t" + offset2str[mention1] + "," + offset2str[mention2]
            #         + "\t" + type1 + "," + type2 + "\t" + seg_offset + "\t" + score[score_idx] + "\n")
            if relation_arg == "Arg-1":
                if results[i] in ERE_to_AIDA_final_ouput:
                    try:
                        output.write(mid2kbid[mention1] + "\t" + ERE_to_AIDA_final_ouput[results[i]] + "\t" + mid2kbid[
                ...
                if results[i] in ERE_to_AIDA_final_ouput and results[i] not in single_ERE:
                    try:
                        output.write(mid2kbid[mention2] + "\t" + ERE_to_AIDA_final_ouput[results[i]] + "\t" + mid2kbid[
